...
Scenario: Assign tests
	...
	And an existing application with test code "22393"
	When the application "22393" is assigned to the qc rule "2238"
	Then the application "22393" and the qc rule "2238" are linked
...
Scenario: Unassign tests
	...
	And an existing application with test code "16201"
	And the application "16201" is assigned to the qc rule "1731"
	When the application "16201" is unassigned to the qc rule "1731"
	Then the application "16201" and the qc rule "1731" are not linked

Scenario: Validate QC results
	Given an existing application with test code "7260"
	And an existing control for the test "7260" with the values
		| Range1SD | TargetValue |
		| 0.5      | 0.9         |
	And a Standard Deviation rule existing in the system with the values
         | Name | WithinControl | Comment | NumberOfControls | StandardDeviationLimits |
         | Rule 606     | True               | Comment 201        | 1                 | 2                        |
	And the application "7260" is assigned to the qc rule "Rule 606"
